...
public static class DbExtensions
{
    ...
    {
        public void Initialize(FormattableString text)
        {
    ...
    extension(DbDataSource dataSource)
    {
        public void ExecuteCommand(FormattableString commandText)
        {
        ...
        public IEnumerable<T> ExecuteCommand<T>(FormattableString commandText, Func<IDatabaseRow, T> mapper)
        {
            using DbConnection connection = dataSource.OpenConnection();
            using DbCommand command = connection.CreateCommand();
            command.Initialize(commandText);
        ...
        public async Task ExecuteCommandAsync(FormattableString commandText, CancellationToken cancellationToken = default)
        {
        ...
        public async IAsyncEnumerable<T> ExecuteCommandAsync<T>(FormattableString commandText, Func<IAsyncDatabaseRow, Task<T>> mapper, [EnumeratorCancellation] CancellationToken cancellationToken = default)
        {
            DbConnection connection = await dataSource.OpenConnectionAsync(cancellationToken).ConfigureAwait(false);
            await using ConfiguredAsyncDisposable configuredConnection = connection.ConfigureAwait(false);
            DbCommand command = connection.CreateCommand();
