...
public sealed class ImageLoader : IImageLoader
{
    ...
    public Task<IImage> LoadImageAsync(Stream stream, CancellationToken cancellationToken = default) => _internalImageLoader.LoadImageAsync(stream, cancellationToken);
    ...
    public Task<IImage> LoadImagesToGridAsync(IEnumerable<Stream?> streams, ImageGridOptions? options = null, CancellationToken cancellationToken = default)
        => _internalImageLoader.LoadImagesToGridAsync(streams, options, cancellationToken);
}
